...
bool PointfootHW::startBipedController() {
  ROS_WARN("Entered startBipedController()");  // ✅ [디버깅용] 진입 확인
...
bool PointfootHW::init(ros::NodeHandle& root_nh, ros::NodeHandle& robot_hw_nh) {
  ...
  // joy_sub_ = root_nh.subscribe<sensor_msgs::Joy>("/virtual_joystick", 10, [this, &root_nh](const sensor_msgs::Joy::ConstPtr& msg) {
  void hw::PointfootHW::joyCallback(const sensor_msgs::Joy::ConstPtr& msg) {

    ROS_INFO_STREAM("Received Joy msg | buttons: " << msg->buttons.size()
                                                  << ", axes: " << msg->axes.size());

    limxsdk::SensorJoy sdk_joy;
    sdk_joy.stamp = msg->header.stamp.toNSec();  // 또는 ros::Time::now().toNSec()
    sdk_joy.buttons = msg->buttons;
    sdk_joy.axes = msg->axes;
  ...
  // robot_->subscribeSensorJoy([this](const limxsdk::SensorJoyConstPtr& msg) {
    ...
    if (joystick_btn_map_.count("L1") > 0 && joystick_btn_map_.count("B") > 0) {
      if (msg->buttons[joystick_btn_map_["L1"]] == 1 && msg->buttons[joystick_btn_map_["B"]] == 1) {
        ...
        //tem // static ros::ServiceClient mode_client = root_nh.serviceClient<robot_controllers::SetMode>("/controller/set_mode");
        static ros::ServiceClient mode_client;
        if (!mode_client) mode_client = root_nh.serviceClient<robot_controllers::SetMode>("/controller/set_mode");
        ...
        // srv.request.data = true;  // true = STAND
        // 25.07.30 [BDS]]
    ...
    if (joystick_btn_map_.count("L1") > 0 && joystick_btn_map_.count("Y") > 0) {
      if (msg->buttons[joystick_btn_map_["L1"]] == 1 && msg->buttons[joystick_btn_map_["Y"]] == 1) {
        ...
        // std_srvs::SetBool srv;
        // srv.request.data = true;  // true = WALK
        // 25.07.30 [BDS]]
        robot_controllers::SetMode srv;
        srv.request.mode = 1;
